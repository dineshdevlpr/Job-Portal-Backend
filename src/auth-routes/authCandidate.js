...
router.post("/register", async (req, res) => {
    try {
      ...
      let data = await db.collection("candidate").findOne({ email: req.body.email });
  
      if (!data) {
        let hashedPassword = await bcrypt.hash(req.body.password, 10);
        req.body.password = hashedPassword;
        let regToken = jwt.sign({email: req.body.email},process.env.REG_SECRET_KEY);
            await db.collection("candidate").insertOne({ firstname:req.body.firstname, lastname:req.body.lastname, degree:req.body.degree, email:req.body.email, password:req.body.password, active:false, regToken:regToken});
  ...
  router.put("/activation/:token", async(req,res)=>{
    try{
        let client = await MongoClient.connect(dbUrl);
        let db = client.db("Job-Portal");
        let data = await db.collection("candidate").findOne({ regToken:req.params.token });
        if(data){
            await db.collection("candidate").updateOne({ regToken:req.params.token },{ $unset : { regToken:1 }, $set : { active:true }});
