...
router.post("/register", async (req, res) => {
    try {
      ...
      let data = await db.collection("recruiter").findOne({ email: req.body.email });
  
      if (!data) {
        let hashedPassword = await bcrypt.hash(req.body.password, 10);
        req.body.password = hashedPassword;
        let regToken = jwt.sign({email: req.body.email},process.env.REG_SECRET_KEY);
            await db.collection("recruiter").insertOne({ firstname:req.body.firstname, lastname:req.body.lastname, email:req.body.email, password:req.body.password, active:false, regToken:regToken});
            ...
            let mailDetails = await mailTransporter.sendMail({
                ...
                html: `<div>
                    ...
                    <a href="${feUrl+"recruiter/activation/"+regToken}">Activate Account</a>
                  </div>`,
              });
  ...
  router.put("/activation/:token", async(req,res)=>{
    try{
        let client = await MongoClient.connect(dbUrl);
        let db = client.db("Job-Portal");
        let data = await db.collection("recruiter").findOne({ regToken:req.params.token });
        if(data){
            await db.collection("recruiter").updateOne({ regToken:req.params.token },{ $unset : { regToken:1 }, $set : { active:true }});
        ...
        let mailTransporter = nodeMailer.createTransport({
         ...
         let mailDetails = await mailTransporter.sendMail({
          ...
          html: `<div>
                    ...
                    <a href="${feUrl+"recruiter/reset/"+randomString}">RESET PASSWORD</a>
                  </div>`,
        })   
        console.log(mailDetails)
        res.status(200).json({
  ...
  router.post("/reset/:randomString", async (req, res) => {
    try {
      ...
      const userData = await db.collection("recruiter").findOne({
